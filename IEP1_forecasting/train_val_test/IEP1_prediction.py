...
from IEP1 import MultiOutputLSTM
import os
...
model.load_state_dict(torch.load("multi_output_lstm.pth"))
model.eval()
...
predicted_df["date"] = date_range
 ...
 # Create output directory
    output_dir = os.path.join(os.getcwd(), "train_val_test", "prediction_plots")
...
for feature, name in zip(selected_features, features_names):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"prediction_plots/{feature}_forecast.png")
    plt.close()
